...
ROOT_DIR = os.getcwd()
VERSION = '5.4.0a0'  # 根据labelme的版本来修改
...
def load_mosaic(imgpath, cls, imgfile, img_size=640):
    ...
    files = np.concatenate([np.array([imgpath]), np.random.choice(imgfile, size=2, replace=False)])
...
class DataAugmentation(object):
    ...
    def resize_(self, img, boxes, size):
        ...
        ow = int(sw * w + 0.5)
        oh = int(sh * h + 0.5)
        for singMark in boxes:
            for singlePolygon in singMark[1:]:
                singlePolygon[0] = singlePolygon[0] * sw
                singlePolygon[1] = singlePolygon[1] * sh
        # label, boxes = boxes[:, :1], boxes[:, 1:5]
        # boxes = boxes * torch.Tensor([sw, sh, sw, sh])
        # boxes = torch.cat((label, boxes), dim=1)
    ...
    def random_flip_horizon(self, img, boxes):
        ...
        if p > 0.5:
            ...
            for singMark in boxes:
                for singlePolygon in singMark[1:]:
                    singlePolygon[0] = w - singlePolygon[0]
        return img, boxes
    ...
    def random_flip_vertical(self, img, boxes):
        ...
        if p > 0.5:
            ...
            for singMark in boxes:
                for singlePolygon in singMark[1:]:
                    singlePolygon[1] = h - singlePolygon[1]
        return img, boxes

    def center_crop(self, img, boxes, size=(200, 200), imgfile=None):
        """
        中心裁剪
        :param img: Image
        :param boxes: bbox坐标
        ...
        if torch.rand(1) > 0.5:
            ...
            for singMark in boxes:
                for singlePolygon in singMark[1:]:
                    singlePolygon[0] = min(max(singlePolygon[0] - j, 0), max_size[0])
                    singlePolygon[1] = min(max(singlePolygon[1] - i, 0), max_size[1])
            res = []
            for singMark in boxes:
                flag = True
                image = cv2.imread('/'.join(imgfile.split(r'\\')))  # （这里读入的图的尺寸要大于你的多边形）
                polygon = np.array(singMark[1:], dtype=np.int32)  # 这里是多边形的顶点坐标
                im = np.zeros(image.shape[:2], dtype="uint8")  # 获取图像的维度: (h,w)=iamge.shape[:2]
                polygon_mask = cv2.fillConvexPoly(im, polygon, 255)
                area = np.sum(np.greater(polygon_mask, 0))
                if area <= 400 and len(list(singMark[1:])) > 1:

                    for singlePolygon in singMark[1:]:
                        if singlePolygon[0] <= 10 or singlePolygon[0] >= max_size[0] - 10 or singlePolygon[1] <= 10 or \
                                singlePolygon[1] >= max_size[1] - 10:
                            continue
                        flag = False
                        break
                    if not flag:
    ...
    def mixup(self, img1, img2, box1, box2, alpha=32.):
        ...
        return self.to_image(miximg), box1 + box2
        # return img1, box1
...
def load_json_points(file, cls):
    ...
    point = []
    maxLen = 0
    for item in doc['shapes']:
        temp = [cls.index(item['label'])]
        for singlePloy in item['points']:
            temp.append(singlePloy)
            # temp.append(singlePloy[0])
            # temp.append(singlePloy[1])
        point.append(temp)
    # tenList = []
    # for p in point:
    #     tenList.append(torch.tensor(p))
    return point
            ...
            for t in range(extimes):
                ...
                for index, funcname in enumerate(method):
                    ...
                    if funcname == 'mixup':
                        ...
                        # imagepath = "D:\selectImg\dataAugTest\IMG_2394_cropped.jpg"
                        print(imagepath)
                    ...
                    elif funcname == 'resize_':
                        image, points = eval(func)(image, points,
                                                   [random.randint(400, 1280), random.randint(400, 1280)])
                    elif funcname == 'center_crop':
                        size = random.randint(150, 300)
                        image, points = eval(func)(image, points, size=(size, size), imgfile=imgfile)
                    else:
                        image, points = eval(func)(image, points)
...
if __name__ == '__main__':
    ...
    extimes表示扩充倍数
    """
    functionList = ['resize', 'resize_', 'random_flip_horizon', 'random_flip_vertical', 'center_crop',
                    'random_equalize',
                    'random_autocontrast', 'random_adjustSharpness', 'random_solarize', 'random_posterize',
                    'random_grayscale', 'gaussian_blur', 'random_invert', 'random_cutout', 'random_erasing',
                    'random_bright', 'random_contrast', 'random_saturation', 'add_gasuss_noise', 'add_salt_noise',
